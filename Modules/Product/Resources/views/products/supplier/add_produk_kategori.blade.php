                        ...
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <div class="col-span-2 bg-transparent">
                                ...
                                @if(strpos($category->category_name, 'Mutiara') !== false)
                                     @include('product::products.supplier.mutiara')
                                @elseif(strpos($category->category_name, 'Berlian') !== false)
                                    @include('product::products.supplier.berlian')
                                @elseif (\Illuminate\Support\Str::contains($category->category_name, ['Perak', 'Paladium']))
                                @include('product::products.supplier.perak')
                                @elseif(strpos($category->category_name, 'Logam Mulia') !== false)
                                @include('product::products.supplier.lm')
                                @elseif (strpos($category->category_name, 'Emas') !== false)
                                  @include('product::products.supplier.emas')
                                @else
                                   @include('product::products.supplier.all')
                                @endif
