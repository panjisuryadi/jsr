<div>
    <div class="card border-0 py-0">
        <div class="card-body px-3 py-1 text-black">
            @include('sale::pos.filter')
            <div class="mt-2 grid grid-cols-4 md:gap-0 lg:gap-1 gap-2 md:grid-cols-4 sm:grid-cols-2 lg:grid-cols-4 relative ">
                ...
                <div wire:click.prevent="selectProduct({{ $product }})" class="md:px-1 py-1" style="cursor: pointer;">
                    ...
                    @endphp
                    <div class="h-40 lg:h-32 md:h-32 relative overflow-hidden rounded-md  hover:shadow-md">
                        <img src="{{ $imagePath }}" class="absolute inset-0 h-full w-full object-cover" />
                        <span style="font-size:0.7rem;" class="absolute top-2 leading-5 left-1 inline-flex items-center justify-center rounded-lg bg-red-400 px-1 py-0.2 text-red-200">
                            {{@$product->karat->label}}
                        </span>
                        <div class="absolute md:bottom-1 bottom-3 md:leading-4 left-2">
