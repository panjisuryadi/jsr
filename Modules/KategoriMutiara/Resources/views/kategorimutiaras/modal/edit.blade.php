  ...
  <form id="FormEdit" action="{{ route(''.$module_name.'.update', $detail) }}" method="POST">
                            @csrf
                            @method('patch')
             <div class="flex flex-row grid grid-cols-1 gap-4 py-2">
            
                <div class="form-group mb-0">
                    <label for="jenis_mutiara_id">Jenis Mutiara <span class="text-danger">*</span></label>
                    <select class="form-control" name="jenis_mutiara_id" id="jenis_mutiara_id" required>
                        @foreach(\Modules\JenisMutiara\Models\JenisMutiara::all() as $main)
                        <option {{ $main->id == $detail->jenis_mutiara_id ? 'selected' : '' }} value="{{ $main->id }}">{{ $main->name }}</option>
                        @endforeach
                    </select>
                </div>
