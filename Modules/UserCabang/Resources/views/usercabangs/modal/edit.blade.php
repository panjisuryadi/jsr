  ...
  <form id="FormEdit" action="{{ route(''.$module_name.'.update', $detail) }}" method="POST">
             ...
             <div class="flex flex-row grid grid-cols-2 gap-4">
                         ...
                         <select name="{{ $field_name }}" id="{{ $field_name }}" class="form-control" required>
                                    @foreach(\app\Models\User::all() as $cab)
                                    <option {{ $detail->user_id == $cab->id ? 'selected' : '' }} value="{{ $cab->id }}">{{ $cab->name }}</option>
                                    @endforeach
                                </select>
                       ...
                       <div class="form-group">
                              ...
                              <select name="{{ $field_name }}" id="{{ $field_name }}" class="form-control" required>
                                    ...
                                    <option {{ $detail->cabang_id == $cab->id ? 'selected' : '' }} value="{{ $cab->id }}">{{ $cab->name }}</option>
                                    @endforeach
                                </select>
                                <span class="invalid feedback" role="alert">
                                    <span class="text-danger error-text {{ $field_name }}_err"></span>
