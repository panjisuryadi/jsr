...
namespace Modules\KeluarMasuk\Http\Controllers;
use Carbon\Carbon;
...
class KeluarMasuksController extends Controller
{

  public function __construct()
    {
        // Page Title
        $this->module_title = 'Keluar Masuk';
        $this->module_name = 'keluarmasuk';
        $this->module_path = 'keluarmasuks';
        $this->module_icon = 'fas fa-sitemap';
        $this->module_model = "Modules\KeluarMasuk\Models\KeluarMasuk";
...
public function index_data(Request $request)

    {
        ...
        return Datatables::of($$module_name)
                        ...
                        ->editColumn('weight_in', function ($data) {
                            $tb = '<div class="items-center text-center">
                                    <h3 class="text-sm font-medium text-gray-800">
                                    ' .$data->weight_in .'</h3>
                                    </div>';
                        ...
                        ->editColumn('weight_out', function ($data) {
                            $tb = '<div class="items-center text-center">
                                    <h3 class="text-sm font-medium text-gray-800">
                                    ' .$data->weight_out .'</h3>
                                    </div>';
                        ...
                        ->editColumn('remaining_weight', function ($data) {
                            $tb = '<div class="items-center text-center">
                                    <h3 class="text-sm font-medium text-gray-800">
                                    ' .$data->remaining_weight .'</h3>
                                    </div>';
                                return $tb;
                            })
                           
                        ->rawColumns([ 'action', 'date','karat','description','weight_in','weight_out','remaining_weight'])
                        ->make(true);
                     }
    ...
    {
         abort_if(Gate::denies('create_keluarmasuk'), 403);
        $module_title = $this->module_title;
        $module_name = $this->module_name;
        $module_path = $this->module_path;
        $module_icon = $this->module_icon;
