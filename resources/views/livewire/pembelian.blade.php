...
@if(!empty($query))
            <div wire:click="resetQuery" class="position-fixed w-100 h-100" style="left: 0; top: 0; right: 0; bottom: 20px;z-index: 0;"></div>
            @if($search_results->isNotEmpty())
            <div class="card position-absolute mt-0 overflow-y-auto h-32"
            style="width: 340px !important;z-index: 3;left: 0;bottom: 2rem;
                    border: 1px solid #f0f0ef !important;">
                <div class="card-body px-2 py-1">
                    <ul class="flex flex-col divide-y divide">
                        @foreach($search_results as $result)
                        <li class="flex flex-row border-top">
 ...
 <div class="position-absolute mt-0"
            style="width: 340px !important;z-index: 3;left: 0;bottom: 3.4rem;
                    border: 1px solid #f0f0ef !important;">
  ...
  <span class="absolute top-0 bottom-0 right-0 px-2 py-3">
    <a  href="{{ route('products.index') }}" target="_blank" class="py-0 btn btn-sm btn-outline-danger mr-1">
        @lang('Add Product')
       </a>
  </span>
</div>
