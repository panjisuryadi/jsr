...
class Create extends Component
{
    ...
    public function render(){
        return view('livewire.stock-opname.kroom.create');
    }
    ...
    public function store(){
        ...
        try{
            ...
            foreach($this->adjustment_items as $item){
                $adjustment->stockKroom()->attach($item['stock_kroom_id'],[
                    'weight_before'=>$item['current_stock'],
                    'weight_after' =>$item['new_stock']
                ]);
                StockKroom::findOrFail($item['stock_kroom_id'])->update(['weight'=>$item['new_stock']]);
            }
            DB::commit();
        }catch (\Exception $e) {
            DB::rollBack(); 
