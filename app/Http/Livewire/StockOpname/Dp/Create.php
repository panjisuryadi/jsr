...
class Create extends Component
{
    ...
    public function render(){
        return view('livewire.stock-opname.dp.create');
    }
    ...
    public function store(){
        ...
        try{
            ...
            foreach($this->adjustment_items as $item){
                $adjustment->stockDP()->attach($item['stock_dp_id'],[
                    'weight_before'=>$item['current_stock'],
                    'weight_after' =>$item['new_stock']
                ]);
                StokDp::findOrFail($item['stock_dp_id'])->update(['weight'=>$item['new_stock']]);
            }
            DB::commit();
        }catch (\Exception $e) {
            DB::rollBack(); 
