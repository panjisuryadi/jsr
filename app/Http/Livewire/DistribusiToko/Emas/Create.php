...
class Create extends Component
{
    ...
    public function render(){
        $data = Product::whereIn('status_id',[ProductStatus::PENDING_OFFICE,ProductStatus::DRAFT]);
    ...
    public function submitBarcode()
    {
        if(!empty($this->kode_produk)){
            $data = Product::where('product_code',$this->kode_produk)->whereIn('status_id',[ProductStatus::PENDING_OFFICE,ProductStatus::DRAFT]);
            if (!empty($this->exceptProductId)) {
                $data = $data->whereNotIn('id', $this->exceptProductId);
            }
            $data = $data->first();
